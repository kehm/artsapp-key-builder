...
/**
 * Render add creator dialog
 */
const AddCreator = ({
    openDialog, onClose, onUpdate, disableText,
}) => {
    ...
    return (
        ...
        >
            <form className="p-2" autoComplete="off" onSubmit={handleSubmit}>
                <DialogTitle>{language.dictionary.headerAddCreator}</DialogTitle>
                <DialogContent>
                    <CloseButton onClick={() => onClose(name !== '')} />
                    {!disableText && (
                        <>
                            <p className="mb-8">{language.dictionary.sectionCreators}</p>
